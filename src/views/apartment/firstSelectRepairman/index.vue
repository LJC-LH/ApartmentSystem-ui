...
<script>
import { listFirstSelectRepairman, getFirstSelectRepairman, delFirstSelectRepairman, addFirstSelectRepairman, updateFirstSelectRepairman } from "@/api/apartment/firstSelectRepairman";

export default {
  name: "FirstSelectRepairman",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFirstSelectRepairman(this.queryParams).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.repairId != null) {
            updateFirstSelectRepairman(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFirstSelectRepairman(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除一次派单编号为"' + repairIds + '"的数据项？').then(function() {
        return delFirstSelectRepairman(repairIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('apartment/firstSelectRepairman/export', {
        ...this.queryParams
      }, `firstSelectRepairman_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
